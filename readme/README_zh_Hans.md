...
`chatflows/JW页分版-SkillAgent-Pro.yml` 是一个面向计算机网络学习的完整 Chatflow：使用单一 Skill Agent 替代分类器树，保留 Dify 最近会话历史，通过百炼记忆持久化学习记录，并由外部 Skills 路由问候、概念教学、解题、批改、出题、下一步学习建议和范围引导。

配套 Skills 位于 `examples/external-skills/`，把其中内容挂载到 `/opt/dify-agent-skills` 即可热加载。
